<template>
  <div class="animate-fade-in">
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-2">Data Mahasiswa</h1>
      <p class="text-gray-600 dark:text-gray-400">Kelola data mahasiswa yang terdaftar</p>
    </div>
    ...
    <div class="grid md:grid-cols-3 gap-6 mb-8">
      <div class="glass rounded-xl p-6 card-hover">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-600 dark:text-gray-400 text-sm font-semibold mb-1">Total Mahasiswa</p>
            <p class="text-3xl font-bold text-gray-800 dark:text-gray-100">{{ students.length }}</p>
          </div>
      ...
      <div class="glass rounded-xl p-6 card-hover">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-600 dark:text-gray-400 text-sm font-semibold mb-1">Terdaftar Hari Ini</p>
            <p class="text-3xl font-bold text-gray-800 dark:text-gray-100">{{ todayRegistrations }}</p>
          </div>
      ...
      <div class="glass rounded-xl p-6 card-hover">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-600 dark:text-gray-400 text-sm font-semibold mb-1">Aktif Bulan Ini</p>
            <p class="text-3xl font-bold text-gray-800 dark:text-gray-100">{{ monthlyActive }}</p>
          </div>
        ...
        <table class="w-full">
          <thead class="bg-gray-100 dark:bg-gray-800">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Tanggal Daftar</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="(student, index) in filteredStudents" :key="student.id" class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ index + 1 }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="w-10 h-10 bg-gradient-to-br from-primary-400 to-purple-500 rounded-full flex items-center justify-center mr-3">
                    <span class="text-white font-semibold">{{ student.name.charAt(0) }}</span>
                  </div>
                  <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ student.name }}</span>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 font-mono">{{ student.nim }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">{{ student.email }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">{{ formatDate(student.createdAt) }}</td>
      ...
      <div @click.stop class="glass rounded-2xl p-8 max-w-md mx-auto mt-32 animate-slide-up">
        <div class="text-center mb-6">
          ...
          <p class="text-gray-600 dark:text-gray-400">
            Apakah Anda yakin ingin menghapus mahasiswa <strong>{{ studentToDelete?.name }}</strong>?
          </p>
        </div>
...
const todayRegistrations = computed(() => {
  ...
  return students.value.filter(s => new Date(s.createdAt).toDateString() === today).length
})
